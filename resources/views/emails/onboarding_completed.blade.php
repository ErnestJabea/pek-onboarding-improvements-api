...
Félicitations, vous avez complété avec succès votre parcours d'onboarding pour le **Fonds Commun de Placement (FCP) PEK**.
...
Pour finaliser définitivement l'ouverture de votre compte titres auprès de **Kori Asset Management**, veuillez préparer et fournir à nos agents les justificatifs physiques suivants :
* 📂 Copie certifiée conforme de votre pièce d'identité en cours de validité ({{ $payload['piece'] ?? 'CNI/Passeport' }} n° {{ $payload['num_piece'] ?? '' }})
...
* 💼 Justificatif de l'origine de vos fonds (bulletin de paie récent, relevé de compte bancaire, ou registre de commerce)
...
* **Profil Investisseur** : {{ $payload['risk_profile'] ?? 'Non calculé' }} (Score : {{ $payload['risk_score'] ?? '0' }}/23)
...
> [!WARNING]
> **ATTENTION** : Ce dossier comporte des alertes LAB-FT. Un examen renforcé est nécessaire avant toute approbation.
@endif
